import { rubik80sFade, orbitron } from "@/lib/googleFonts";
import SkillsGlobe from "./SkillsGlobe";
import { Badge } from "./ui/badge";

function Skills() {
  const skills = [
    {
      name: "mongodb",
      color: "currentColor",
      svgString: `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 32 32"><path d="M15.821 23.185s0-10.361.344-10.36c.266 0 .612 13.365.612 13.365-.476-.056-.956-2.199-.956-3.005zm6.668-10.24c-.919-4.016-2.932-7.469-5.708-10.134l-.007-.006a9.8 9.8 0 0 1-.895-1.732l-.024-.068.001.068c0 .565-.253 1.07-.652 1.409l-.003.002c-3.574 3.034-5.848 7.505-5.923 12.508v.013l-.001.208a15.1 15.1 0 0 0 6.07 12.115l.039.028.087.063q.241 1.784.412 3.576h.601c.166-1.491.39-2.796.683-4.076l-.046.239c.396-.275.742-.56 1.065-.869l-.003.003a14.143 14.143 0 0 0 4.549-10.404l-.001-.182v.009a16.341 16.341 0 0 0-.261-2.871l.015.099z"/></svg>`, // Your full SVG string here
    },
    {
      name: "react",
      color: "currentColor",
      svgString: `<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 16 16"><g fill="#000"><path d="M4.85 2.372c-.251.145-.515.544-.546 1.416a.75.75 0 0 1-1.499-.053c.038-1.068.376-2.131 1.295-2.662.922-.532 2.015-.29 2.961.214a.75.75 0 0 1-.706 1.324c-.773-.413-1.253-.384-1.505-.24Zm4.021 1.3a.75.75 0 0 1 1.056.1c.555.669 1.087 1.432 1.57 2.268 1.04 1.802 1.681 3.648 1.854 5.196.086.773.06 1.509-.12 2.14-.183.635-.537 1.21-1.131 1.553-.719.415-1.55.357-2.309.08-.762-.278-1.547-.803-2.295-1.486a.75.75 0 0 1 1.011-1.108c.66.602 1.279.996 1.797 1.184.522.19.85.145 1.046.031.164-.095.33-.288.438-.667.11-.384.145-.91.072-1.56-.145-1.299-.7-2.947-1.663-4.613A14.994 14.994 0 0 0 8.772 4.73a.75.75 0 0 1 .1-1.056Z"/><path d="M14.5 8.206c0-.29-.214-.719-.953-1.18a.75.75 0 0 1 .795-1.273C15.248 6.32 16 7.144 16 8.206c0 1.064-.756 1.89-1.666 2.457a.75.75 0 0 1-.793-1.273c.744-.464.959-.894.959-1.184Zm-3.137 2.832a.75.75 0 0 1-.614.865c-.857.145-1.784.224-2.749.224-2.08 0-4-.367-5.427-.992-.713-.311-1.336-.702-1.792-1.174C.32 9.486 0 8.891 0 8.206c0-.83.466-1.52 1.085-2.04.622-.521 1.47-.938 2.435-1.244a.75.75 0 0 1 .454 1.43c-.85.27-1.502.609-1.924.964-.426.356-.55.663-.55.89 0 .19.085.43.359.713.278.287.716.58 1.315.842 1.197.524 2.902.866 4.826.866.885 0 1.727-.072 2.498-.203a.75.75 0 0 1 .865.614Z"/><path d="M4.872 13.63c.25.145.729.174 1.499-.235a.75.75 0 1 1 .704 1.325c-.944.501-2.034.74-2.953.21-.922-.533-1.26-1.6-1.295-2.672a.75.75 0 0 1 1.499-.05c.03.875.294 1.277.546 1.422Zm-.884-4.133a.75.75 0 0 1-.442-.964c.302-.815.697-1.657 1.18-2.493 1.04-1.802 2.318-3.28 3.572-4.204.626-.461 1.276-.806 1.913-.965.641-.16 1.317-.141 1.91.202.72.415 1.084 1.163 1.224 1.96.141.799.078 1.741-.14 2.73a.75.75 0 1 1-1.464-.321c.191-.872.223-1.606.127-2.15-.096-.546-.3-.807-.496-.92-.164-.095-.415-.142-.797-.046-.388.097-.86.33-1.387.718-1.052.774-2.202 2.08-3.163 3.746-.443.766-.801 1.532-1.073 2.265a.75.75 0 0 1-.964.442Z"/><path d="M9.46 8.066a1.36 1.36 0 1 1-2.72 0 1.36 1.36 0 0 1 2.72 0Z"/></g></svg>`,
    },
    {
      name: "next",
      color: "currentColor",
      svgString: `<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"><g clip-path="url(#a)"><path fill="#000" d="M11.214.006c-.052.005-.216.022-.364.033C7.442.347 4.25 2.186 2.226 5.013a11.88 11.88 0 0 0-2.118 5.243c-.096.66-.108.854-.108 1.748s.012 1.089.108 1.748c.652 4.507 3.86 8.293 8.209 9.696.779.251 1.6.422 2.533.526.364.04 1.936.04 2.3 0 1.611-.179 2.977-.578 4.323-1.265.207-.105.247-.134.219-.157-.02-.014-.899-1.194-1.955-2.62l-1.919-2.593-2.404-3.559a342.499 342.499 0 0 0-2.422-3.556c-.009-.003-.018 1.578-.023 3.51-.007 3.38-.01 3.516-.052 3.596a.426.426 0 0 1-.206.213c-.075.038-.14.045-.495.045H7.81l-.108-.068a.44.44 0 0 1-.157-.172l-.05-.105.005-4.704.007-4.706.073-.092a.644.644 0 0 1 .174-.143c.096-.047.133-.051.54-.051.478 0 .558.018.682.154.035.038 1.337 2 2.895 4.362l4.734 7.172 1.9 2.878.097-.063a12.318 12.318 0 0 0 2.465-2.163 11.947 11.947 0 0 0 2.825-6.135c.096-.66.108-.854.108-1.748s-.012-1.088-.108-1.748C23.24 5.75 20.032 1.963 15.683.56a12.6 12.6 0 0 0-2.498-.523c-.226-.024-1.776-.05-1.97-.03Zm4.913 7.26a.473.473 0 0 1 .237.276c.018.06.023 1.365.018 4.305l-.007 4.218-.743-1.14-.746-1.14v-3.066c0-1.983.009-3.097.023-3.151a.478.478 0 0 1 .232-.296c.097-.05.132-.054.5-.054.347 0 .408.005.486.047Z"/></g><defs><clipPath id="a"><path fill="#fff" d="M0 0h24v24H0z"/></clipPath></defs></svg>`,
    },
    {
      name: "git",
      color: "",
      svgString: `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 32 32"><path d="M30.428 14.663 17.333 1.569c-.35-.349-.833-.565-1.367-.565s-1.017.216-1.367.565l-2.713 2.718 3.449 3.449a2.296 2.296 0 0 1 2.903 2.942l.005-.016 3.322 3.324a2.296 2.296 0 1 1-1.374 1.288l-.006.015-3.11-3.094v8.154a2.3 2.3 0 1 1-2.65.438c.211-.211.462-.382.741-.502l.015-.006v-8.234a2.305 2.305 0 0 1-1.239-3.027l-.006.015-3.389-3.405-8.98 8.974c-.348.351-.562.834-.562 1.368s.215 1.017.563 1.368L14.664 30.43c.349.35.832.566 1.366.566s1.016-.216 1.366-.566L30.43 17.396a1.925 1.925 0 0 0 0-2.732z"/></svg>`,
    },
    {
      name: "nodeJs",
      color: "",
      svgString: `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 448 512"><!--!Font Awesome Free 6.7.2 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license/free Copyright 2025 Fonticons, Inc.--><path d="M224 508c-6.7 0-13.5-1.8-19.4-5.2l-61.7-36.5c-9.2-5.2-4.7-7-1.7-8 12.3-4.3 14.8-5.2 27.9-12.7 1.4-.8 3.2-.5 4.6.4l47.4 28.1c1.7 1 4.1 1 5.7 0l184.7-106.6c1.7-1 2.8-3 2.8-5V149.3c0-2.1-1.1-4-2.9-5.1L226.8 37.7c-1.7-1-4-1-5.7 0L36.6 144.3c-1.8 1-2.9 3-2.9 5.1v213.1c0 2 1.1 4 2.9 4.9l50.6 29.2c27.5 13.7 44.3-2.4 44.3-18.7V167.5c0-3 2.4-5.3 5.4-5.3h23.4c2.9 0 5.4 2.3 5.4 5.3V378c0 36.6-20 57.6-54.7 57.6-10.7 0-19.1 0-42.5-11.6l-48.4-27.9C8.1 389.2.7 376.3.7 362.4V149.3c0-13.8 7.4-26.8 19.4-33.7L204.6 9c11.7-6.6 27.2-6.6 38.8 0l184.7 106.7c12 6.9 19.4 19.8 19.4 33.7v213.1c0 13.8-7.4 26.7-19.4 33.7L243.4 502.8c-5.9 3.4-12.6 5.2-19.4 5.2zm149.1-210.1c0-39.9-27-50.5-83.7-58-57.4-7.6-63.2-11.5-63.2-24.9 0-11.1 4.9-25.9 47.4-25.9 37.9 0 51.9 8.2 57.7 33.8.5 2.4 2.7 4.2 5.2 4.2h24c1.5 0 2.9-.6 3.9-1.7s1.5-2.6 1.4-4.1c-3.7-44.1-33-64.6-92.2-64.6-52.7 0-84.1 22.2-84.1 59.5 0 40.4 31.3 51.6 81.8 56.6 60.5 5.9 65.2 14.8 65.2 26.7 0 20.6-16.6 29.4-55.5 29.4-48.9 0-59.6-12.3-63.2-36.6-.4-2.6-2.6-4.5-5.3-4.5h-23.9c-3 0-5.3 2.4-5.3 5.3 0 31.1 16.9 68.2 97.8 68.2 58.4-.1 92-23.2 92-63.4z"/></svg>`,
    },
    {
      name: "C++",
      color: "",
      svgString: `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 32 32"><path d="M26.914 13.8v1.54h-1.542v1.543h1.542v1.543h1.543v-1.543H30V15.34h-1.543V13.8Zm-3.5 0H21.87v1.54h-1.543v1.543h1.543v1.543h1.543v-1.543h1.543V15.34h-1.543Zm-3.654 5.226a6.167 6.167 0 1 1-.04-6.118l5.39-3.1a12.384 12.384 0 1 0 .032 12.34Z" style="fill:#000"/></svg>`,
    },
    {
      name: "Java",
      color: "",
      svgString: `<svg xmlns="http://www.w3.org/2000/svg" xml:space="preserve" viewBox="0 0 512 512"><path d="M193.918 208.369c-4.729-10.456-6.849-22.652-3.236-33.731 3.612-11.327 11.703-20.413 19.794-28.878 22.525-22.531 50.285-39.085 72.316-61.986 12.197-12.573 22.278-27.634 25.762-44.937 2.864-12.695 1.496-25.764-1.117-38.337 11.7 13.319 15.559 32.363 12.197 49.541-3.608 19.292-14.316 36.344-26.886 51.031-10.081 11.827-21.659 22.282-33.731 31.993-14.065 11.327-27.88 23.524-36.716 39.457-7.472 12.943-9.215 28.876-4.11 42.942 8.341 24.146 27.756 42.071 38.338 64.848-11.703-10.332-23.152-21.036-33.86-32.361-11.198-11.95-21.78-24.644-28.751-39.582zm63.48-18.921c-2.115 19.792 8.213 38.462 20.539 53.151 5.972 6.596 11.076 14.687 11.323 23.899.251 12.318-6.716 23.774-15.684 31.861 2.119-.246 3.612-2.115 5.355-3.11 13.443-8.589 26.385-19.418 32.982-34.227 4.357-10.083 3.362-22.034-2.362-31.371-6.724-10.953-15.559-20.662-20.786-32.61-2.867-6.721-3.862-14.562-1.496-21.657 3.114-9.583 9.834-17.426 16.93-24.272 19.54-18.544 43.189-31.743 65.844-46.179-28.629 8.214-56.883 19.542-81.03 37.343-15.311 11.451-29.498 27.382-31.615 47.172zm136.049 93.604c13.568.748 26.882 10.704 29.374 24.397 2.366 11.825-3.358 23.524-10.705 32.485-12.075 14.438-28.382 24.771-44.807 33.609-1.622.991-2.99 2.237-4.235 3.608 21.659-5.478 43.314-13.689 60.867-27.756 9.705-8.091 18.294-18.799 20.163-31.619 1.743-11.076-2.86-22.528-11.077-29.871-9.96-9.09-24.021-12.448-37.218-10.704-7.593.995-15.931 2.613-21.158 8.961 6.226-1.241 12.32-3.733 18.796-3.11zM123.22 318.647c16.303 4.357 33.108 5.603 49.787 6.724 14.936.995 29.875 1.246 44.937 1.12 38.833-.619 77.916-3.236 116.003-11.699 3.608-.87 7.593-1.493 10.833-3.733 6.347-4.11 13.313-7.347 20.162-10.583-30.995 4.979-62.113 9.215-93.478 11.205-31.74 1.991-63.731 3.236-95.593 1.121-9.086-.87-18.423-1.371-26.886-4.858-1.994-.87-4.733-2.609-3.738-5.227 1.869-3.361 5.603-4.977 8.839-6.72 13.694-6.473 28.629-10.081 43.193-14.313-25.021-.376-49.916 5.971-72.814 15.806-5.105 2.491-10.83 4.481-14.936 8.714-1.622 1.739-1.622 4.732.247 6.222 3.735 3.361 8.711 4.854 13.444 6.221zm201.644 34.233c-21.784 3.859-43.694 7.472-65.726 8.589-24.147 1.618-48.294.247-72.191-2.241-4.604-.623-9.211-1.368-13.317-3.483-2.116-1.246-4.231-3.236-4.106-5.854.247-4.106 3.73-6.967 6.222-9.956-7.715 2.739-15.434 5.599-21.906 10.708-2.742 2.116-5.478 5.474-4.733 9.208 1.125 4.356 5.356 6.97 9.09 8.96 9.208 4.733 19.54 6.846 29.625 8.714 25.511 4.11 51.527 4.235 77.167 2.488 27.141-2.115 54.148-6.594 80.411-14.313-7.468-3.358-14.564-7.221-20.536-12.82zm-136.796 43.071c-6.97 1.99-14.066 4.357-19.79 8.957-2.868 2.241-5.105 6.104-3.734 9.713 1.743 4.604 6.1 7.347 10.203 9.705 10.708 5.854 22.78 8.589 34.604 10.708 26.765 4.229 54.27 3.608 80.908-1.12 15.806-2.989 31.615-7.221 46.301-13.815-9.584-3.984-18.917-8.467-27.878-13.693-15.559 2.738-31.246 5.603-47.178 6.598-21.032 1.618-42.319-.125-63.355-2.738-4.98-1.121-11.202-1.618-14.563-5.976-1.739-3.613 2.242-6.227 4.482-8.339zm170.277 80.031c17.424-3.604 34.977-7.719 50.908-15.806 4.976-2.867 11.076-5.979 12.698-11.95.87-5.725-5.105-8.714-9.337-11.08 2.613 2.993 4.356 7.347 1.74 10.83-4.357 5.853-11.821 8.091-18.42 10.332-20.66 5.85-42.072 8.216-63.355 10.582-56.385 5.102-113.146 6.348-169.528 1.618-18.92-1.994-38.217-4.109-56.264-10.829-2.86-1.246-7.217-2.492-7.217-6.352 1.117-3.354 4.357-5.227 7.217-6.845 12.945-6.595 27.384-10.207 41.822-11.077-4.228-2.491-9.208-2.738-14.062-2.613-12.076.373-23.9 3.483-35.349 7.347-9.834 3.604-19.916 7.59-27.76 14.811-3.111 2.735-5.971 7.962-2.739 11.699 4.98 5.353 12.699 6.72 19.54 8.338 38.338 6.599 77.171 10.328 116.011 11.699 51.531 1.498 103.434-.744 154.095-10.704zm51.033 6.724c-23.402 7.468-47.672 11.574-71.822 14.936-41.696 5.227-83.769 6.845-125.716 5.603-25.515-.995-51.03-2.738-76.176-6.974 5.85 3.984 13.071 5.227 19.794 7.096 28.257 5.976 57.255 7.096 86.01 7.966 42.19.748 84.387-.87 125.962-7.468 19.669-3.48 39.459-7.715 57.13-16.927 9.215-4.854 18.552-12.326 20.163-23.152-9.332 9.955-22.772 14.563-35.345 18.92z"/></svg>`,
    },
    {
      name: "Mysql",
      color: "",
      svgString: `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path d="m24.129 23.412-.508-.484a6.66 6.66 0 0 0-.809-.891l-.005-.004q-.448-.407-.931-.774-.387-.266-1.064-.641a1.6 1.6 0 0 1-.818-.824l-.004-.01-.048-.024c.212-.021.406-.06.592-.115l-.023.006.57-.157c.236-.074.509-.122.792-.133h.006c.298-.012.579-.06.847-.139l-.025.006q.194-.048.399-.109t.351-.109v-.169q-.145-.217-.351-.496a2.561 2.561 0 0 0-.443-.468l-.005-.004q-.629-.556-1.303-1.076a15.783 15.783 0 0 0-1.311-.916l-.068-.04a4.983 4.983 0 0 0-.825-.435l-.034-.012q-.448-.182-.883-.399a1.638 1.638 0 0 0-.327-.119l-.011-.002a.516.516 0 0 1-.29-.169l-.001-.001a2.999 2.999 0 0 1-.355-.609l-.008-.02q-.145-.339-.314-.651-.363-.702-.702-1.427t-.651-1.452q-.217-.484-.399-.967a5.395 5.395 0 0 0-.461-.942l.013.023a17.366 17.366 0 0 0-1.331-1.961l.028.038a14.57 14.57 0 0 0-1.459-1.59l-.008-.007a17.619 17.619 0 0 0-1.632-1.356l-.049-.035q-.896-.651-1.96-1.282a3.7 3.7 0 0 0-.965-.393l-.026-.006-1.113-.278-.629-.048q-.314-.024-.629-.024a1.68 1.68 0 0 1-.387-.279 3.581 3.581 0 0 0-.353-.295l-.01-.007a11.686 11.686 0 0 0-2.043-.93L2.071.18A1.355 1.355 0 0 0 .9.096L.909.093a1.36 1.36 0 0 0-.795.84l-.003.01a1.515 1.515 0 0 0 .232 1.549l-.002-.003q.544.725.834 1.14.217.291.448.605c.141.188.266.403.367.63l.008.021c.056.119.105.261.141.407l.003.016q.048.206.121.448.217.556.411 1.14c.141.425.297.785.478 1.128l-.019-.04q.145.266.291.52T3.738 9a.868.868 0 0 0 .241.242l.003.002a.406.406 0 0 1 .169.313v.001a1.326 1.326 0 0 0-.217.586l-.001.006a4.227 4.227 0 0 1-.153.695l.008-.03a7.103 7.103 0 0 0-.351 2.231c0 .258.013.512.04.763l-.003-.031c.06.958.349 1.838.812 2.6l-.014-.025c.197.295.408.552.641.787a.914.914 0 0 0 1.106.203l-.005.002a.926.926 0 0 0 .617-.827v-.002c.048-.474.12-.898.219-1.312l-.013.067a.595.595 0 0 0 .038-.211l-.002-.045v.002q-.012-.109.133-.206v.048q.145.339.302.677t.326.677c.295.449.608.841.952 1.202l-.003-.003a7.74 7.74 0 0 0 1.127 1.001l.022.015c.212.162.398.337.566.528l.004.004c.158.186.347.339.56.454l.01.005v-.024h.048a.461.461 0 0 0-.18-.205l-.002-.001a1.829 1.829 0 0 1-.211-.136l.005.003q-.217-.217-.448-.484t-.423-.508q-.508-.702-.969-1.467t-.871-1.555q-.194-.387-.375-.798t-.351-.798a.997.997 0 0 1-.096-.334v-.005a.318.318 0 0 0-.168-.265l-.002-.001a2.947 2.947 0 0 1-.408.545l.001-.001a1.974 1.974 0 0 0-.382.58l-.005.013a4.272 4.272 0 0 0-.289 1.154l-.002.019q-.072.641-.145 1.318l-.048.024-.024.024a.855.855 0 0 1-.59-.443l-.002-.005q-.182-.351-.326-.69a6.448 6.448 0 0 1-.423-2.144v-.009a6.218 6.218 0 0 1 .286-2.318l-.012.044q.072-.266.314-.896t.097-.871a.876.876 0 0 0-.265-.41l-.001-.001a3.31 3.31 0 0 1-.335-.335l-.003-.004q-.169-.244-.326-.52t-.278-.544a11.408 11.408 0 0 1-.474-1.353l-.022-.089a10.174 10.174 0 0 0-.546-1.503l.026.064a3.32 3.32 0 0 0-.39-.669l.006.008q-.244-.326-.436-.617-.244-.314-.484-.605a3.414 3.414 0 0 1-.426-.657l-.009-.02a1.638 1.638 0 0 1-.119-.327l-.002-.011a.406.406 0 0 1 .049-.34l-.001.002a.303.303 0 0 1 .073-.145.308.308 0 0 1 .143-.072h.002a.55.55 0 0 1 .536-.035l-.003-.001c.219.062.396.124.569.195l-.036-.013q.459.194.847.375c.298.142.552.292.792.459l-.018-.012q.194.121.387.266t.411.291h.339q.387 0 .822.037c.293.023.564.078.822.164l-.024-.007c.481.143.894.312 1.286.515l-.041-.019q.593.302 1.125.641c.589.367 1.098.743 1.577 1.154l-.017-.014c.5.428.954.867 1.38 1.331l.01.012c.416.454.813.947 1.176 1.464l.031.047c.334.472.671 1.018.974 1.584l.042.085a4.6 4.6 0 0 1 .234.536l.011.033q.097.278.217.57.266.605.57 1.221t.57 1.198l.532 1.161c.187.406.396.756.639 1.079l-.011-.015c.203.217.474.369.778.422l.008.001c.368.092.678.196.978.319l-.047-.017c.143.065.327.134.516.195l.04.011c.212.065.396.151.565.259l-.009-.005c.327.183.604.363.868.559l-.021-.015q.411.302.822.57.194.145.651.423t.484.52a11.202 11.202 0 0 0-1.834.087l.056-.006a5.959 5.959 0 0 0-1.479.39l.04-.014a2.556 2.556 0 0 1-.388.129l-.019.004a.312.312 0 0 0-.266.277v.001c.061.076.11.164.143.26l.002.006c.034.102.075.19.125.272l-.003-.006c.119.211.247.393.391.561l-.004-.005c.141.174.3.325.476.454l.007.005q.244.194.508.399c.161.126.343.25.532.362l.024.013c.284.174.614.34.958.479l.046.016c.374.15.695.324.993.531l-.016-.011q.291.169.58.375t.556.399c.073.072.137.152.191.239l.003.005a.573.573 0 0 0 .36.193h.003v-.048a.516.516 0 0 1-.184-.267l-.001-.004a.95.95 0 0 0-.112-.273l.002.004zM5.553 4.207q-.194 0-.363.012a1.285 1.285 0 0 0-.323.063l.009-.003v.024h.048q.097.145.244.326t.266.351l.387.798.048-.024a.735.735 0 0 0 .252-.321l.002-.005c.052-.139.082-.301.082-.469l-.001-.054v.003a.504.504 0 0 1-.108-.154l-.001-.003-.081-.182a.52.52 0 0 0-.214-.192l-.003-.001a.93.93 0 0 1-.244-.169z"/></svg>`,
    },
    {
      name: "Postgresql",
      color: "",
      svgString: `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 32 32"><path d="M24.295 9.929a.56.56 0 0 1-.191.387l-.001.001a.894.894 0 0 1-.504.278l-.005.001a.68.68 0 0 1-.094.007h-.004a.752.752 0 0 1-.694-.46l-.002-.005c-.03-.221.33-.388.701-.44s.764.011.794.231zm-9.91.514a.822.822 0 0 1-.766.528h-.011.001a.91.91 0 0 1-.105-.007h.004a.997.997 0 0 1-.63-.392l-.002-.003a.539.539 0 0 1-.132-.354v-.001a.278.278 0 0 1 .115-.185l.001-.001a1.094 1.094 0 0 1 .77-.107l-.007-.001c.395.055.803.242.764.523zm11.018 7.643-.107-.134-.044-.055a6.164 6.164 0 0 0 .725-2.921c0-.488-.056-.962-.162-1.418l.008.042a10.08 10.08 0 0 1-.111-1.508v-.021.001a10.9 10.9 0 0 1 .158-1.429l-.01.066A8.35 8.35 0 0 0 26 9.165l-.001-.151v.008a.686.686 0 0 0 .019-.162c0-.027-.002-.053-.004-.079v.003a9.49 9.49 0 0 0-2.168-4.073l.007.008a9.795 9.795 0 0 0-3.3-2.523l-.06-.025a11.255 11.255 0 0 1 2.316-.234l.225.002h-.011l.15-.002a7.466 7.466 0 0 1 5.866 2.838l.012.016a.962.962 0 0 1 .08.12l.003.005c.904 1.694-.345 7.842-3.732 13.172zm-.286-8.764a9.111 9.111 0 0 1-.14 1.307l.008-.055c-.078.425-.134.931-.157 1.445l-.001.025v.056c0 .567.042 1.124.124 1.668l-.008-.061c.085.377.134.809.134 1.254 0 .763-.144 1.493-.407 2.162l.014-.04a4.07 4.07 0 0 1-.224-.453l-.011-.029a9.727 9.727 0 0 0-.406-.77c-.769-1.38-2.571-4.611-1.649-5.929.474-.678 1.676-.707 2.722-.579zm-.711 11.585c-.051-1.039.336-1.148.746-1.263q.085-.023.169-.051c.05.044.105.087.162.125l.005.003c.62.273 1.342.431 2.102.431.592 0 1.161-.096 1.693-.274l-.038.011a5.005 5.005 0 0 1-1.16.738l-.031.013a6.31 6.31 0 0 1-2.173.454h-.01a2.734 2.734 0 0 1-1.482-.195l.018.007zm-1.113 2.026c-.021.221-.045.47-.077.745l-.182.548a.437.437 0 0 0-.023.134l.001.089c0 .355-.053.699-.151 1.022l.006-.025c-.116.389-.196.84-.223 1.305l-.001.016a3.307 3.307 0 0 1-3.008 3.194l-.013.001c-1.894.406-2.23-.621-2.526-1.527q-.045-.142-.096-.283a9.25 9.25 0 0 1-.252-2.171c0-.36.02-.715.059-1.065l-.004.043a10.001 10.001 0 0 0-.432-3.378l.019.071q.008-.55.024-1.114v-.012a.444.444 0 0 0-.02-.132l.001.003a1.872 1.872 0 0 0-.058-.273l.003.013a1.806 1.806 0 0 0-.964-1.164l-.011-.005a1.358 1.358 0 0 0-.907-.107l.009-.002c.128-.47.26-.854.412-1.228l-.026.073.066-.177c.074-.2.167-.407.266-.626a8.827 8.827 0 0 0 .865-3.841c0-.938-.144-1.842-.411-2.692l.017.063a2.314 2.314 0 0 0-2.805-1.883l.016-.003a6.412 6.412 0 0 0-2.171.682l.035-.017q-.124.063-.245.13a9.286 9.286 0 0 1 2.181-5.615l-.012.014c.118-.119.242-.232.37-.338l.009-.007a.45.45 0 0 0 .182-.081l-.001.001a5.455 5.455 0 0 1 3.515-1.04l-.013-.001c.526.008 1.034.044 1.534.108l-.067-.007a8.618 8.618 0 0 1 5.032 2.963l.011.014a9.864 9.864 0 0 1 1.766 2.998l.022.069a3.399 3.399 0 0 0-3.346.972l-.002.002c-1.24 1.773.679 5.215 1.601 6.869.169.303.315.565.361.676.26.601.587 1.118.98 1.577l-.007-.008c.087.109.171.214.236.306-.501.144-1.401.478-1.319 2.146-.015.195-.053.558-.104 1.018a7.887 7.887 0 0 0-.123.93l-.002.028zm-9.202-5.714-.066.176c-.137.328-.279.745-.397 1.172l-.019.081a3.147 3.147 0 0 1-2.261-1.001l-.002-.002a3.716 3.716 0 0 1-.975-3.146l-.003.021c.097-.72.153-1.551.153-2.396 0-.502-.02-.999-.058-1.491l.004.065c-.006-.107-.012-.201-.015-.275a4.838 4.838 0 0 1 3.32-.965l-.017-.001c.554.129.971.588 1.037 1.153l.001.006c.238.728.375 1.566.375 2.435a8.07 8.07 0 0 1-.809 3.532l.021-.048c-.105.233-.204.453-.289.682zm-2.617 4.984a2 2 0 0 1-.549-.227l.009.005c.172-.073.375-.134.585-.173l.019-.003c1.604-.33 1.851-.563 2.392-1.25.124-.157.264-.336.459-.553a.44.44 0 0 0 .091-.159l.001-.003c.213-.189.34-.137.546-.052a.942.942 0 0 1 .46.587l.001.007a.654.654 0 0 1-.058.558l.002-.003a3.507 3.507 0 0 1-3.983 1.258l.025.007zm-7.507-6.357c-.651-1.985-1.181-4.34-1.494-6.764l-.021-.199a5.87 5.87 0 0 1-.095-1.071c0-1.806.789-3.427 2.041-4.537l.006-.005c2.295-1.623 6.048-.676 7.633-.163l-.012.012a10.904 10.904 0 0 0-2.463 7.198v-.013c0 .103.008.249.02.449a18.133 18.133 0 0 1-.102 3.738l.01-.091a4.593 4.593 0 0 0 1.218 3.863l-.002-.002q.151.158.315.297a64.838 64.838 0 0 0-2.377 2.696c-.709.853-1.199.689-1.36.636a3.505 3.505 0 0 1-1.538-1.631l-.009-.022a21.48 21.48 0 0 1-1.73-4.232l-.039-.157zm26.478 3.557a.66.66 0 0 0-.072-.151l.002.003c-.174-.329-.596-.427-1.259-.29-2.066.426-2.866.164-3.156-.024a29.312 29.312 0 0 0 3.751-8.326l.049-.209c.339-1.313.997-4.403.153-5.913a1.982 1.982 0 0 0-.192-.298l.003.004a8.452 8.452 0 0 0-6.85-3.193h.011l-.141-.001c-1.342 0-2.633.22-3.838.625l.085-.025q-.321-.06-.645-.102a9.694 9.694 0 0 0-1.631-.16h-.008a6.406 6.406 0 0 0-3.835 1.065l.024-.015c-1.071-.401-5.984-2.056-9.025.098a6.868 6.868 0 0 0-2.324 6.432l-.006-.041a40.078 40.078 0 0 0 1.632 7.461l-.079-.284a23.079 23.079 0 0 0 1.852 4.6l-.061-.124a4.48 4.48 0 0 0 2.116 2.225l.026.012a2.062 2.062 0 0 0 2.317-.901l.005-.009a69.459 69.459 0 0 1 2.43-2.758 3.82 3.82 0 0 0 1.731.471h.006l.001.005q-.158.188-.309.382c-.424.538-.512.649-1.875.93-.388.08-1.418.292-1.433 1.014v.013c0 .147.043.284.117.399l-.002-.003c.312.395.751.678 1.254.788l.015.003c.399.13.859.205 1.335.205 1.1 0 2.106-.398 2.884-1.058l-.006.005a26.627 26.627 0 0 0 .459 6.526l-.027-.167a3.233 3.233 0 0 0 3.084 2.38h.002a5.012 5.012 0 0 0 1.07-.124l-.033.006a3.92 3.92 0 0 0 3.567-3.619l.001-.012c.188-1.088.502-3.593.673-5.125a.341.341 0 0 1 .072-.171l-.001.001c.001-.001.087-.059.534.038l.055.009.317.028.019.001a7.654 7.654 0 0 0 3.214-.557l-.051.018c.805-.373 2.256-1.29 1.993-2.087z"/></svg>`,
    },
    {
      name: "HTML",
      color: "",
      svgString: `<svg xmlns="http://www.w3.org/2000/svg" xml:space="preserve" viewBox="0 0 512 512"><path d="M30.713.501 71.717 460.42l184.006 51.078 184.515-51.15L481.287.501H30.713zm365.041 109.145-2.567 28.596-1.128 12.681H176.204l5.155 57.761h205.565l-1.377 15.146-13.255 148.506-.849 9.523L256 413.854v.012l-.259.072-115.547-32.078-7.903-88.566H188.915l4.016 44.986 62.82 16.965.052-.014v-.006l62.916-16.977 6.542-73.158h-195.49l-13.863-155.444-1.351-15.131h282.547l-1.35 15.131z"/></svg>`,
    },
    {
      name: "vscode",
      color: "",
      svgString: `<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"><g fill="#000"><path d="M.228 8.37s-.584-.427.117-.995L1.98 5.897s.467-.497.962-.064l15.081 11.542v5.534s-.007.87-1.11.774L.227 8.369Z"/><path d="M4.116 11.937.228 15.509s-.4.3 0 .837l1.805 1.66s.429.465 1.062-.065l4.121-3.158-3.1-2.846ZM10.94 11.966l7.13-5.502-.047-5.505s-.305-1.202-1.32-.576L7.216 9.11l3.724 2.856Z"/><path d="M16.912 23.69c.414.428.916.288.916.288l5.556-2.767c.711-.49.611-1.098.611-1.098V3.588c0-.726-.735-.977-.735-.977L18.444.264c-1.052-.657-1.741.119-1.741.119s.886-.645 1.32.576v21.85c0 .15-.032.297-.095.43-.127.259-.402.5-1.062.4l.046.051Z"/></g></svg>`,
    },
    {
      name: "intellij",
      color: "",
      svgString: `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 32 32"><path d="M0 0v32h32V0zm4.964 4.145h6.667v2.448H9.776v8.371h1.855v2.443H4.964v-2.443h1.927V6.593H4.964zm14.739 0h2.964v8.667c0 .819-.073 1.485-.297 2.079a4.002 4.002 0 0 1-.963 1.479c-.371.371-.885.745-1.48.891a5.192 5.192 0 0 1-1.927.371c-1.036 0-1.927-.224-2.593-.595a6.403 6.403 0 0 1-1.704-1.405l1.855-2.073c.369.443.74.74 1.109.964.369.219.812.369 1.26.369.521 0 .959-.151 1.333-.521.292-.369.443-.891.443-1.703zM2.964 26h12v2h-12z"/></svg>`,
    },
    {
      name: "Typescript",
      color: "",
      svgString: `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path d="M0 12v12h24V0H0zm19.341-.956c.61.152 1.074.423 1.501.865.221.236.549.666.575.77.008.03-1.036.73-1.668 1.123-.023.015-.115-.084-.217-.236-.31-.45-.633-.644-1.128-.678-.728-.05-1.196.331-1.192.967a.88.88 0 0 0 .102.45c.16.331.458.53 1.39.933 1.719.74 2.454 1.227 2.911 1.92.51.773.625 2.008.278 2.926-.38.998-1.325 1.676-2.655 1.9-.411.073-1.386.062-1.828-.018-.964-.172-1.878-.648-2.442-1.273-.221-.243-.652-.88-.625-.925a1.58 1.58 0 0 1 .22-.141c.108-.061.511-.294.892-.515l.69-.4.145.214c.202.308.643.731.91.872.766.404 1.817.347 2.335-.118a.883.883 0 0 0 .313-.72c0-.278-.035-.4-.18-.61-.186-.266-.567-.49-1.649-.96-1.238-.533-1.771-.864-2.259-1.39a3.165 3.165 0 0 1-.659-1.2c-.091-.339-.114-1.189-.042-1.531.255-1.197 1.158-2.03 2.461-2.278.423-.08 1.406-.05 1.821.053zm-5.634 1.002.008.983H10.59v8.876H8.38v-8.876H5.258v-.964c0-.534.011-.98.026-.99.012-.016 1.913-.024 4.217-.02l4.195.012z"/></svg>`,
    },
    {
      name: "Fastapi",
      color: "",
      svgString: `<svg fill="#000000" viewBox="0 0 24 24" role="img" xmlns="http://www.w3.org/2000/svg"><g id="SVGRepo_bgCarrier" stroke-width="0"></g><g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g><g id="SVGRepo_iconCarrier"><path d="M12 0C5.375 0 0 5.375 0 12c0 6.627 5.375 12 12 12 6.626 0 12-5.373 12-12 0-6.625-5.373-12-12-12zm-.624 21.62v-7.528H7.19L13.203 2.38v7.528h4.029L11.376 21.62z"></path></g></svg>`,
    },
    {
      name: "postman",
      color: "",
      svgString: `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path d="M13.527.099C6.955-.744.942 3.9.099 10.473c-.843 6.572 3.8 12.584 10.373 13.428 6.573.843 12.587-3.801 13.428-10.374C24.744 6.955 20.101.943 13.527.099zm2.471 7.485a.855.855 0 0 0-.593.25l-4.453 4.453-.307-.307-.643-.643c4.389-4.376 5.18-4.418 5.996-3.753zm-4.863 4.861 4.44-4.44a.62.62 0 1 1 .847.903l-4.699 4.125-.588-.588zm.33.694-1.1.238a.06.06 0 0 1-.067-.032.06.06 0 0 1 .01-.073l.645-.645.512.512zm-2.803-.459 1.172-1.172.879.878-1.979.426a.074.074 0 0 1-.085-.039.072.072 0 0 1 .013-.093zm-3.646 6.058a.076.076 0 0 1-.069-.083.077.077 0 0 1 .022-.046h.002l.946-.946 1.222 1.222-2.123-.147zm2.425-1.256a.228.228 0 0 0-.117.256l.203.865a.125.125 0 0 1-.211.117h-.003l-.934-.934-.294-.295 3.762-3.758 1.82-.393.874.874c-1.255 1.102-2.971 2.201-5.1 3.268zm5.279-3.428h-.002l-.839-.839 4.699-4.125a.952.952 0 0 0 .119-.127c-.148 1.345-2.029 3.245-3.977 5.091zm3.657-6.46-.003-.002a1.822 1.822 0 0 1 2.459-2.684l-1.61 1.613a.119.119 0 0 0 0 .169l1.247 1.247a1.817 1.817 0 0 1-2.093-.343zm2.578 0a1.714 1.714 0 0 1-.271.218h-.001l-1.207-1.207 1.533-1.533c.661.72.637 1.832-.054 2.522zm-.1-1.544a.143.143 0 0 0-.053.157.416.416 0 0 1-.053.45.14.14 0 0 0 .023.197.141.141 0 0 0 .084.03.14.14 0 0 0 .106-.05.691.691 0 0 0 .087-.751.138.138 0 0 0-.194-.033z"/></svg>`,
    },
    {
      name: "prisma",
      color: "",
      svgString: `<svg viewBox="-27 0 310 310" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" preserveAspectRatio="xMidYMid" fill="#000000"><g id="SVGRepo_bgCarrier" stroke-width="0"></g><g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g><g id="SVGRepo_iconCarrier"> <g> <path d="M254.312882,235.518775 L148.000961,9.74987264 C145.309805,4.08935083 139.731924,0.359884549 133.472618,0.0359753113 C127.198908,-0.384374336 121.212054,2.71925839 117.939655,8.08838662 L2.63252565,194.847143 C-0.947129465,200.604248 -0.871814894,207.912774 2.8257217,213.594888 L59.2003287,300.896318 C63.5805009,307.626626 71.8662281,310.673635 79.5631922,308.384597 L243.161606,259.992851 C248.145475,258.535702 252.252801,254.989363 254.421072,250.271225 C256.559881,245.57581 256.523135,240.176915 254.32061,235.511047 L254.312882,235.518775 Z M230.511129,245.201761 L91.6881763,286.252058 C87.4533189,287.511696 83.388474,283.840971 84.269448,279.567474 L133.866738,42.0831633 C134.794079,37.6396542 140.929985,36.9364206 142.869673,41.0476325 L234.684164,236.021085 C235.505704,237.779423 235.515611,239.809427 234.711272,241.575701 C233.906934,243.341974 232.369115,244.667163 230.503401,245.201761 L230.511129,245.201761 Z" fill="#000000" fill-rule="nonzero"> </path> </g> </g></svg>`,
    },
  ];

  const langs = [
    "C++",
    "Java",
    "JavaScript",
    "TypeScript",
    "Python",
    "HTML",
    "CSS",
  ];
  const dbs = [
    "SQL",
    "Mysql",
    "PostgreSQL",
    "MongoDB",
    "Supabase",
    "Firebase",
    "Prisma",
    "Drizzle",
  ];

  const framworks = [
    "React",
    "Next.js",
    "Node.js",
    "Express.js",
    "Tailwind CSS",
    "Bootstrap",
    "FastAPI",
  ];
  const tools = [
    "Git",
    "VS Code",
    "IntelliJ IDEA",
    "PyCharm",
    "Postman",
    "Jira",
    "Slack",
    "Figma",
  ];
  return (
    <div className="min-h-screen">
      <div className="flex items-center justify-center w-full relative">
        <span
          className={`absolute top-20 left-8 md:left-2 text-7xl sm:text-8xl md:text-9xl opacity-30  text-foreground select-none ${rubik80sFade.className}`}
        >
          SKILLS
        </span>

        <div className="w-full flex items-center justify-center relative">
          <div className="flex flex-col lg:flex-row justify-between items-center w-full min-h-screen">
            {/* Skills on the left */}
            <div className="w-[90%] lg:w-1/2 h-full">
              <div className="grid gap-3 pt-16 sm:pt-24 md:pt-30 px-4 sm:px-6 md:px-10">
                {/* Languages */}
                <div className="flex flex-col justify-start items-center w-full relative px-4 pt-16">
                  <div className="flex items-start w-full">
                    <h3
                      className={`text-3xl font-semibold text-foreground ${orbitron.className}`}
                    >
                      Languages
                    </h3>
                  </div>
                  <div className="w-full flex flex-wrap gap-2 pt-6 select-none">
                    {langs.map((language, index) => (
                      <div
                        key={index}
                        className="relative group inline-flex items-center justify-center mb-2"
                      >
                        <svg
                          className="absolute inset-0 w-full h-full"
                          viewBox="0 0 120 40"
                          preserveAspectRatio="none"
                        >
                          <rect
                            x="2"
                            y="2"
                            width="116"
                            height="36"
                            rx="8"
                            ry="8"
                            fill="none"
                            stroke="white"
                            strokeWidth="1"
                            className="draw-ring"
                          />
                        </svg>
                        <Badge
                          className="text-sm bg-foreground/20 text-foreground hover:shadow-2xl shadow-foreground transition-colors duration-300 ease-in-out"
                          variant="outline"
                        >
                          {language}
                        </Badge>
                      </div>
                    ))}
                  </div>
                </div>
                <div className="flex flex-col justify-start items-center w-full relative px-4">
                  <div className="flex items-start w-full">
                    <h3
                      className={`text-3xl font-semibold text-foreground ${orbitron.className}`}
                    >
                      Database & tools
                    </h3>
                  </div>
                  <div className="w-full flex flex-wrap gap-2 pt-6 select-none">
                    {dbs.map((dbtool, index) => (
                      <div
                        key={index}
                        className="relative group inline-flex items-center justify-center mb-2"
                      >
                        <svg
                          className="absolute inset-0 w-full h-full"
                          viewBox="0 0 120 40"
                          preserveAspectRatio="none"
                        >
                          <rect
                            x="2"
                            y="2"
                            width="116"
                            height="36"
                            rx="8"
                            ry="8"
                            fill="none"
                            stroke="white"
                            strokeWidth="1"
                            className="draw-ring"
                          />
                        </svg>
                        <Badge
                          className="text-sm bg-foreground/20 text-foreground hover:shadow-2xl shadow-foreground transition-colors duration-300 ease-in-out"
                          variant="outline"
                        >
                          {dbtool}
                        </Badge>
                      </div>
                    ))}
                  </div>
                </div>
                {/* Frameworks */}
                <div className="flex flex-col justify-start items-center w-full relative px-4">
                  <div className="flex items-center gap-4 w-full">
                    <h3
                      className={`text-2xl font-semibold text-foreground ${orbitron.className}`}
                    >
                      Frameworks & tools
                    </h3>
                  </div>
                  <div className="w-full flex flex-wrap gap-2 pt-4 select-none">
                    {framworks.map((framework, index) => (
                      <div
                        key={index}
                        className="relative group inline-flex items-center justify-center mb-2"
                      >
                        <svg
                          className="absolute inset-0 w-full h-full"
                          viewBox="0 0 120 40"
                          preserveAspectRatio="none"
                        >
                          <rect
                            x="2"
                            y="2"
                            width="116"
                            height="36"
                            rx="8"
                            ry="8"
                            fill="none"
                            stroke="white"
                            strokeWidth="1"
                            className="draw-ring"
                          />
                        </svg>
                        <Badge
                          className="text-sm bg-foreground/20 text-foreground hover:shadow-2xl shadow-foreground transition-colors duration-300 ease-in-out"
                          variant="outline"
                        >
                          {framework}
                        </Badge>
                      </div>
                    ))}
                  </div>
                </div>

                {/* Tools */}
                <div className="flex flex-col justify-start items-center w-full relative px-4">
                  <div className="flex items-start w-full">
                    <h3
                      className={`text-2xl font-semibold text-foreground ${orbitron.className}`}
                    >
                      Project Tools
                    </h3>
                  </div>
                  <div className="w-full flex flex-wrap gap-2 pt-4 select-none">
                    {tools.map((tool, index) => (
                      <div
                        key={index}
                        className="relative group inline-flex items-center justify-center mb-2"
                      >
                        <svg
                          className="absolute inset-0 w-full h-full"
                          viewBox="0 0 120 40"
                          preserveAspectRatio="none"
                        >
                          <rect
                            x="2"
                            y="2"
                            width="116"
                            height="36"
                            rx="8"
                            ry="8"
                            fill="none"
                            stroke="white"
                            strokeWidth="1"
                            className="draw-ring"
                          />
                        </svg>
                        <Badge
                          className="text-sm bg-foreground/20 text-foreground hover:shadow-2xl shadow-foreground transition-colors duration-300 ease-in-out"
                          variant="outline"
                        >
                          {tool}
                        </Badge>
                      </div>
                    ))}
                  </div>
                </div>
              </div>
            </div>

            {/* Canvas on the right */}
            <div className="w-full lg:w-1/2 h-auto md:h-[80vh] lg:h-screen flex justify-center items-center">
              <SkillsGlobe
                skills={skills}
                className="size-96 sm:size-lvh lg:size-lvh"
              />
            </div>
          </div>
        </div>
      </div>
    </div>
  );
}

export default Skills;
